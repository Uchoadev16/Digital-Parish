    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="bg-white rounded-2xl p-8 golden-glow border border-accent/10 mb-6">
            <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                ...
                <form id="formFotoPerfil" action="../controllers/controller_auth.php" method="post" enctype="multipart/form-data" class="hidden">
                    <input type="hidden" name="alterar_foto" value="1">
                    <input type="file" id="fotoInput" name="foto_perfil" accept="image/jpeg,image/png,image/gif,image/webp" onchange="previewFoto(this)">
                </form>
            </div>

            <!-- Preview e Botão de Enviar -->
            <div id="fotoPreviewContainer" class="hidden mt-6">
                <div class="bg-accent/10 rounded-xl p-6 border-2 border-accent/20">
                    <div class="flex items-center gap-4">
                        <div class="w-24 h-24 rounded-full overflow-hidden border-2 border-accent/30 flex-shrink-0">
                            <img id="fotoPreview" src="" alt="Preview" class="w-full h-full object-cover">
                        </div>
                        <div class="flex-1">
                            <p id="nomeArquivo" class="text-sm font-medium text-primary mb-3"></p>
                            <div class="flex gap-2">
                                <button type="button" onclick="enviarFoto()" class="btn-primary text-white px-6 py-2 rounded-xl font-semibold text-sm">
                                    Enviar Foto
                                </button>
                                <button type="button" onclick="cancelarFoto()" class="px-6 py-2 rounded-xl font-semibold text-sm border-2 border-gray-200 text-primary hover:bg-gray-50">
                                    Cancelar
                                </button>
                            </div>
                        </div>
        ...
        function previewFoto(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById('nomeArquivo').textContent = 'Arquivo selecionado: ' + file.name;
